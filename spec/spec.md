...
~ [Commit history](https://github.com/decentralized-identity/delegated-authority-threat-model/commits/master)
...
Credential lifecycle is not mission lifecycle. A token refreshed at 3 AM does not re-authorize the mission provisioned at 9 AM. Dick Hardt’s [internet draft for the AAuth Protocol](https://datatracker.ietf.org/doc/draft-hardt-oauth-aauth-protocol/) introduces the **mission object** as a first-class protocol artifact — a significant step toward mission lifecycle specification. The gap this paper identifies is the additional layer that evaluates whether the mission remains consistent with subject agency state and intent continuity at execution time. AAuth does not constrain downstream authorization to a subset of upstream scope. The draft states this as a design choice rather than an omission: formal subset rules would prevent legitimate delegation chains, because a downstream resource may need operations orthogonal to anything held upstream — the draft's own example is a booking API calling a payment processor to charge a card, an operation it describes as one the user and the original agent could never perform directly. Downstream scope is instead constrained by the downstream resource's own policy and by the Person Server's evaluation of each hop against mission context. AAuth is therefore not an attenuation architecture with the constraint moved elsewhere: downstream authority is not derived from any party's upstream holdings. The two approaches fail differently. A constraint carried on the credential travels with it and is evaluable wherever the credential is presented. A constraint applied at the Person Server does not travel, and the availability and currency of that server become governance dependencies in their own right. The governance semantics above either approach remain unspecified. [GNAP](https://datatracker.ietf.org/group/gnap/documents/)'s ongoing access model is the closest existing protocol concept to execution-time evaluation but stops short of specifying what must be checked at the moment of action. OAuth's grant lifecycle provides foundational machinery. Neither specifies when a mission should end independent of credential validity.
...
**Attenuation** — The governance requirement that each hop in a delegation chain can narrow scope. A sub-delegate can never get permissions from the delegator that the delegator does not have. A delegate can never get permissions from the subject that the subject does not have. A delegate may combine delegated authority with permissions they independently hold — but cannot pass authority they do not hold. Without enforced attenuation every delegation hop is a potential amplification point.
